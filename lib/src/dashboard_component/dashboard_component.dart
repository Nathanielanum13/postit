...
import 'package:angular_app/src/dashboard_component/create_post_component/create_post_component.dart'
    deferred as create_post_page;
import 'package:angular_app/src/dashboard_component/view_post_component/view_post_component.dart'
    deferred as view_post_page;
import 'package:angular_app/src/dashboard_component/manage_post_component/manage_post_component.dart'
    deferred as manage_post_page;
import 'package:angular_app/src/dashboard_component/dash_home_component/dash_home_component.dart'
    deferred as dash_home_page;
import 'package:angular_app/src/dashboard_component/post_account_component/post_account_component.dart'
    deferred as post_account;
import 'package:angular_app/src/dashboard_component/setting_component/setting_component.dart'
    deferred as settings;
import 'package:angular_app/src/dashboard_component/fb_component/fb_component.dart'
    deferred as fb;
import 'package:angular_app/src/dashboard_component/setting_component/user_account_component/user_account_component.dart'
    deferred as user;
...
class DashboardComponent implements OnInit, CanActivate {
  ...
  void toggleDropdwn() {
    dropdown = !dropdown;
    if (dropdown) {
      getDocument().getElementById('dropdwn-content-temp').style.display =
          'block';
    } else {
      getDocument().getElementById('dropdwn-content-temp').style.display =
          'none';
  ...
  void toggleWidth() {
    ...
    if (isExpanded) {
      getDocument()
          .getElementById('persistent-expandable-drawer')
          .classes
          .remove('small-drawer');
      getDocument()
          .getElementById('persistent-expandable-drawer')
  ...
  void togglePerWidth() {
    ...
    if (isPerExpanded) {
      getDocument()
          .getElementById('permanent-expandable-drawer')
          .classes
          .remove('small-drawer');
    } else {
      getDocument()
  ...
  Future<bool> canActivate(RouterState current, RouterState next) async {
    print(window.sessionStorage['token']);
    print(window.sessionStorage['tenant-namespace']);

    if (window.sessionStorage.containsKey('token') &&
